...
const ChiTietPhieuDatSan = () => {
  ...
  const columns = [
    ...
    {
      name: "Khung giờ",
      selector: (row) => row["Khunggio"].thoigianbatdau + " - " + row["Khunggio"].thoigianketthuc,
      sortable: true,
    },
  ];
  return (
    <>
      <div className="container">
        <h3>Chi tiết lịch sử đặt sân</h3>
        <Link to={`/phieudatsan`} className="btn btn-primary">
          Quay lại
        </Link>
        <DataTable columns={columns} data={details} pagination></DataTable>
      </div>
    </>
  );
};
